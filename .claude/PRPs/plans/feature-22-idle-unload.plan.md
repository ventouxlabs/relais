...
- **MEASURED (rango, Pixel 10 Pro Fold / Tensor G5, gemma-4-E2B, 2026-09-20, `IdleUnloadProbe` test b): `RELOAD_TO_FIRST_TOKEN_MS = 19 836`.** That does **not** materially exceed the ~23 s spike figure, so by this task's own rule the outcome is the *otherwise* branch: **document the hold as deliberate and change nothing.** `/v1/audio/transcriptions` keeps its kick-and-503. The bounded-init-wait design below stays specified-but-unbuilt for a future model/device where the number is materially larger (E4B on G5 is the one to re-measure). Log evidence: `probe-logcat.txt` in the PR-A SDD workspace.
- **ACTION:** ~~Decide, from the measured reload time, whether the synchronous hold needs a deadline.~~ Decided above.
...
2. ~~Stepper ladder and the 1-minute floor.~~ **DECIDED (JD, 2026-09-07): non-linear ladder 1 / 5 / 15
   ...
   mechanism.
3. ~~**`/v1/audio/transcriptions` — bounded-hold or keep 503?**~~ **RESOLVED by measurement (2026-09-20): keep 503** — the reload is ~19.8 s on rango/E2B, under the threshold, so there is no bounded wait for audio to share. (Was: carried from the 09-07 handoff's *still open* list; needs an on-device number, which task 5 produces. Tasks 1–3 do not wait on it; task 6 does, and task 6's spec now says the two share the pre-commit wait or audio keeps its 503 — either is a one-line decision once the number exists.)
5. ~~**Split this step into two PRs?**~~ **DECIDED (JD, 2026-09-20): split as proposed below.** Issues filed the same day so each PR has `Closes` targets: **#333** watchdog shield (4(g), PR-A) · **#334** LAN rebind gate (4(g), PR-A) · **#335** idle survives STOP (4(a), PR-A) · **#336** tile/panel START bounces listeners (4(c), PR-B) · **#337** Configure pick while idle (follow-up, NOT this plan). After the 2026-09-20 round Task 4 is 22 files: the state model (`RelaisLivenessState`, `RelaisEngine.ensureInitialized`, `NodeState`, controller, watchdog, `/health`) plus six reader surfaces (dashboard, experiments, control panel, shell VM, Configure, tile, widget). Tasks 1–3 (metrics, config UI) do not depend on it. The build order's rule is one PR per step, and feature-09 and feature-18 both split when they grew like this. Proposed: **PR-A = Tasks 1, 2, 4(a)(b)(d)(g) + the probe (5)** — engine, liveness, `/health`, watchdog, metrics; **PR-B = Tasks 3, 4(c)(e)(f), 7** — every UI surface plus the Configure controls; **Task 6 after PR-A's measurement.** JD's call.
